...
res = [re.compile('.*Epoch\[(\d+)\] .*accuracy.*=([.\d]+)'),
       re.compile('.*Epoch\[(\d+)\] Palidation-accuracy.*=([.\d]+)')]
...
#    plt.plot(x_train, train_acc, '-', linestyle='-', color=color, linewidth=2, label=val_name)
